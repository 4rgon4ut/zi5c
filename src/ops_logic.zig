...
pub const I_OP = *const fn (cpu: *CPU, rd: u5, rs1: u5, imm: i32) ?Trap;

// --- OP-IMM Instructions (Opcode 0x13) ---

/// Executes ADDI (Add Immediate) instruction: rd = rs1 + sign_extend(imm)
pub fn ADDI(cpu: *CPU, rd: u5, rs1: u5, imm: i32) ?Trap {
...
pub fn SLTI(cpu: *CPU, rd: u5, rs1: u5, imm: i32) ?Trap {
    const val = cpu.readReg(rs1);
...
pub fn SLTIU(cpu: *CPU, rd: u5, rs1: u5, imm: i32) ?Trap {
    const val = cpu.readReg(rs1);
    const imm_u32 = @as(u32, @bitCast(imm));
...
pub fn XORI(cpu: *CPU, rd: u5, rs1: u5, imm: i32) ?Trap {
    const val = cpu.readReg(rs1);
    const imm_u32 = @as(u32, @bitCast(imm));
    cpu.writeReg(rd, val ^ imm_u32);
    cpu.pc +%= 4;
...
pub fn ORI(cpu: *CPU, rd: u5, rs1: u5, imm: i32) ?Trap {
    const val = cpu.readReg(rs1);
    const imm_u32 = @as(u32, @bitCast(imm));
    cpu.writeReg(rd, val | imm_u32);
    cpu.pc +%= 4;
...
pub fn ANDI(cpu: *CPU, rd: u5, rs1: u5, imm: i32) ?Trap {
    const val = cpu.readReg(rs1);
    const imm_u32 = @as(u32, @bitCast(imm));
    cpu.writeReg(rd, val & imm_u32);
    cpu.pc +%= 4;
...
pub fn SLLI(cpu: *CPU, rd: u5, rs1: u5, imm: i32) ?Trap {
    const val = cpu.readReg(rs1);
...
pub fn SRLI(cpu: *CPU, rd: u5, rs1: u5, imm: i32) ?Trap {
    const val = cpu.readReg(rs1);
...
/// Note: shamt is the lower 5 bits of imm. Requires funct7 check in dispatcher.
pub fn SRAI(cpu: *CPU, rd: u5, rs1: u5, imm: i32) ?Trap {
    const val = cpu.readReg(rs1);
    const shamt = @as(u5, @intCast(imm & 0x1F));
...
pub fn LB(cpu: *CPU, rd: u5, rs1: u5, imm: i32) ?Trap {
    const base_addr = cpu.readReg(rs1);
    const offset = @as(u32, @bitCast(imm));
    const mem_addr = base_addr +% offset;
    const byte_val = cpu.ram.readByte(mem_addr) catch |err| {
...
pub fn LH(cpu: *CPU, rd: u5, rs1: u5, imm: i32) ?Trap {
    const base_addr = cpu.readReg(rs1);
    const offset = @as(u32, @bitCast(imm));
    const mem_addr = base_addr +% offset;
    const half_val = cpu.ram.readHalfWord(mem_addr) catch |err| {
        return Trap{ .Fatal = err };
    };
...
pub fn LW(cpu: *CPU, rd: u5, rs1: u5, imm: i32) ?Trap {
    const base_addr = cpu.readReg(rs1);
    const offset = @as(u32, @bitCast(imm));
    const mem_addr = base_addr +% offset;
    const word_val = cpu.ram.readWord(mem_addr) catch |err| {
        return Trap{ .Fatal = err };
    };
...
pub fn LBU(cpu: *CPU, rd: u5, rs1: u5, imm: i32) ?Trap {
    const base_addr = cpu.readReg(rs1);
    const offset = @as(u32, @bitCast(imm));
    const mem_addr = base_addr +% offset;
    const byte_val = cpu.ram.readByte(mem_addr) catch |err| {
...
pub fn LHU(cpu: *CPU, rd: u5, rs1: u5, imm: i32) ?Trap {
    const base_addr = cpu.readReg(rs1);
    const offset = @as(u32, @bitCast(imm));
    const mem_addr = base_addr +% offset;
    const half_val = cpu.ram.readHalfWord(mem_addr) catch |err| {
...
pub fn JALR(cpu: *CPU, rd: u5, rs1: u5, imm: i32) ?Trap {
    const val = cpu.readReg(rs1);
    const imm_u32 = @as(u32, @bitCast(imm));
...
pub fn ECALL(cpu: *CPU, rd: u5, rs1: u5, imm: i32) ?Trap {
    _ = cpu; // autofix
    _ = rd; // autofix
    _ = rs1; // autofix
    _ = imm; // autofix
    return Trap{ .Requested = RequestedTrap.ECALL };
}

pub fn EBREAK(cpu: *CPU, rd: u5, rs1: u5, imm: i32) ?Trap {
    _ = cpu; // autofix
    _ = rd; // autofix
    _ = rs1; // autofix
    _ = imm; // autofix
    return Trap{ .Requested = RequestedTrap.EBREAK };
}

// --- R-Type Execution Functions (Opcode 0x33) ---

pub const R_OP = *const fn (cpu: *CPU, rd: u5, rs1: u5, rs2: u5) ?Trap;

/// Executes ADD instruction: rd = rs1 + rs2
pub fn ADD(cpu: *CPU, rd: u5, rs1: u5, rs2: u5) ?Trap {
...
pub fn SUB(cpu: *CPU, rd: u5, rs1: u5, rs2: u5) ?Trap {
    const val_rs1 = cpu.readReg(rs1);
    const val_rs2 = cpu.readReg(rs2);
...
pub fn SLL(cpu: *CPU, rd: u5, rs1: u5, rs2: u5) ?Trap {
    const val_rs1 = cpu.readReg(rs1);
    const val_rs2 = cpu.readReg(rs2);
...
pub fn SLT(cpu: *CPU, rd: u5, rs1: u5, rs2: u5) ?Trap {
...
pub fn SLTU(cpu: *CPU, rd: u5, rs1: u5, rs2: u5) ?Trap {
...
pub fn XOR(cpu: *CPU, rd: u5, rs1: u5, rs2: u5) ?Trap {
    const val_rs1 = cpu.readReg(rs1);
    const val_rs2 = cpu.readReg(rs2);
    cpu.writeReg(rd, val_rs1 ^ val_rs2);
    cpu.pc +%= 4;
...
pub fn SRL(cpu: *CPU, rd: u5, rs1: u5, rs2: u5) ?Trap {
    const val_rs1 = cpu.readReg(rs1);
    const val_rs2 = cpu.readReg(rs2);
    const shamt = @as(u5, @truncate(val_rs2)); // Shift amount is lower 5 bits of rs2
...
pub fn SRA(cpu: *CPU, rd: u5, rs1: u5, rs2: u5) ?Trap {
    const val_rs1 = cpu.readReg(rs1);
    const val_rs2 = cpu.readReg(rs2);
    const shamt = @as(u5, @truncate(val_rs2)); // Shift amount is lower 5 bits of rs2
...
pub fn OR(cpu: *CPU, rd: u5, rs1: u5, rs2: u5) ?Trap {
    const val_rs1 = cpu.readReg(rs1);
    const val_rs2 = cpu.readReg(rs2);
    cpu.writeReg(rd, val_rs1 | val_rs2);
    cpu.pc +%= 4;
...
pub fn AND(cpu: *CPU, rd: u5, rs1: u5, rs2: u5) ?Trap {
    const val_rs1 = cpu.readReg(rs1);
    const val_rs2 = cpu.readReg(rs2);
    cpu.writeReg(rd, val_rs1 & val_rs2);
    cpu.pc +%= 4;
...
pub fn SH(cpu: *CPU, rs1: u5, rs2: u5, imm: i32) ?Trap {
    const base_addr = cpu.readReg(rs1);
    const offset = @as(u32, @bitCast(imm)); // sign-extended offset
    const mem_addr = base_addr +% offset;
    const value_to_store = @as(u16, @truncate(cpu.readReg(rs2))); // Lower 16 bits
    cpu.ram.writeHalfWord(mem_addr, value_to_store) catch |err| {
        return Trap{ .Fatal = err };
    };
    cpu.pc +%= 4;
    return null;
...
pub fn SW(cpu: *CPU, rs1: u5, rs2: u5, imm: i32) ?Trap {
    const base_addr = cpu.readReg(rs1);
    const offset = @as(u32, @bitCast(imm)); // sign-extended offset
    const mem_addr = base_addr +% offset;
    const value_to_store = cpu.readReg(rs2); // Full 32 bits
    cpu.ram.writeWord(mem_addr, value_to_store) catch |err| {
        return Trap{ .Fatal = err };
    };
    cpu.pc +%= 4;
    return null;
...
pub const B_OP = *const fn (cpu: *CPU, rs1: u5, rs2: u5, imm: i32) ?Trap;

/// Executes BEQ (Branch if Equal) instruction: if (rs1 == rs2) pc += offset
pub fn BEQ(cpu: *CPU, rs1: u5, rs2: u5, imm: i32) ?Trap {
    const val_rs1 = cpu.readReg(rs1);
    const val_rs2 = cpu.readReg(rs2);
...
pub fn BNE(cpu: *CPU, rs1: u5, rs2: u5, imm: i32) ?Trap {
    const val_rs1 = cpu.readReg(rs1);
    const val_rs2 = cpu.readReg(rs2);
    const offset = @as(u32, @bitCast(imm));
    if (val_rs1 != val_rs2) {
...
pub fn BLT(cpu: *CPU, rs1: u5, rs2: u5, imm: i32) ?Trap {
    const val_rs1 = @as(i32, @bitCast(cpu.readReg(rs1))); // Signed comparison
    const val_rs2 = @as(i32, @bitCast(cpu.readReg(rs2)));
    const offset = @as(u32, @bitCast(imm));
    if (val_rs1 < val_rs2) {
...
pub fn BGE(cpu: *CPU, rs1: u5, rs2: u5, imm: i32) ?Trap {
    const val_rs1 = @as(i32, @bitCast(cpu.readReg(rs1))); // Signed comparison
    const val_rs2 = @as(i32, @bitCast(cpu.readReg(rs2)));
    const offset = @as(u32, @bitCast(imm));
    if (val_rs1 >= val_rs2) {
...
pub fn BLTU(cpu: *CPU, rs1: u5, rs2: u5, imm: i32) ?Trap {
    const val_rs1 = cpu.readReg(rs1); // Unsigned comparison
    const val_rs2 = cpu.readReg(rs2);
    const offset = @as(u32, @bitCast(imm));
    if (val_rs1 < val_rs2) {
...
pub fn BGEU(cpu: *CPU, rs1: u5, rs2: u5, imm: i32) ?Trap {
    const val_rs1 = cpu.readReg(rs1); // Unsigned comparison
    const val_rs2 = cpu.readReg(rs2);
    const offset = @as(u32, @bitCast(imm));
    if (val_rs1 >= val_rs2) {
...
pub const U_OP = *const fn (cpu: *CPU, rd: u5, imm: i32) ?Trap;

/// Executes LUI (Load Upper Immediate) instruction: rd = imm << 12
pub fn LUI(cpu: *CPU, rd: u5, imm: i32) ?Trap {
...
pub fn AUIPC(cpu: *CPU, rd: u5, imm: i32) ?Trap {
    // The immediate 'imm' extracted by the decoder for U-type
    // already represents the value with the lower 12 bits as zero.
...
pub const J_OP = *const fn (cpu: *CPU, rd: u5, imm: i32) ?Trap;

/// Executes JAL (Jump and Link) instruction: rd = pc + 4; pc += offset
pub fn JAL(cpu: *CPU, rd: u5, imm: i32) ?Trap {
